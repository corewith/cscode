...
namespace EMMS.DAL
{
    ...
    public class Department
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 no, name from tb_department ");
            strSql.Append("where no=@no ");
        ...
        //得到一个对象实体
        public EMMS.Model.Department DataRowToModel(DataRow row)
        {
            EMMS.Model.Department model = new Model.Department();
            if (row != null)
            {
        ...
        public int AddList(List<EMMS.Model.Department> modelList)
        {
            ...
            //foreach (EMMS.Model.Department model in modelList)
            //{
            //    StringBuilder strSql = new StringBuilder();
            //    strSql.Append("insert into tb_department( ");
            //    strSql.Append("no,name) ");
            ...
            {
                StringBuilder strSql = new StringBuilder();
                strSql.Append("insert into tb_department(no,name) values( '" + model.No + "','" + model.Name + "')");
