...
namespace EMMS.DAL
{
    ...
    public class Role
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 no, name from tb_role ");
            strSql.Append("where no=@no ");

            SqlParameter[] parameters = { new SqlParameter("@no", SqlDbType.VarChar, 2) };
            parameters[0].Value = no;

            //EMMS.Model.Role model = new Model.Role();
        ...
        //得到一个对象实体
        public EMMS.Model.Role DataRowToModel(DataRow row)
        {
            EMMS.Model.Role model = new Model.Role();
            if (row != null)
            {
        ...
        public int AddList(List<EMMS.Model.Role> modelList)
        {
            ...
            foreach (EMMS.Model.Role model in modelList)
            {
                StringBuilder strSql = new StringBuilder();
                strSql.Append("insert into tb_role(no,name) values( '" + model.No + "','" + model.Name + "')");
